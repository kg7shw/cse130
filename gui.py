...
def populate_main_window(frm_main):
    ...
    Return: nothing
    """
    # Create a label that displays "hieght:"
    ...
    ent_height = nent.IntEntry(frm_main, 0, 1000, width=5)
    ...
    lbl_calc = tk.Label(frm_main, width=4)
    ...
    lbl_calc.grid( row=0, column=3, padx=3, pady=3)
    ...
    def calculate(event):
        ...
        """
        try:
            # Get the user's hieght.
            height = ent_height.get()
            width = ent_width.get()

            area = height * width

            lbl_calc.config(text=f"{area:.0f}")
            

        except ValueError:
            # When the user deletes all the digits in the height
    ...
    # Bind the calculate function to the height entry box
    ...
    # Give the keyboard focus to the height entry box.
    ent_height.focus()
    ent_width.focus()
